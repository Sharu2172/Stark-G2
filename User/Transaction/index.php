    ...
    <table class="table table-responsive text-center table-hover w-100">
        ...
        <tbody>
            <form action="../bill/index.php" method="POST">
                ...
                if (isset($_POST["date"])) {
                    $query = "SELECT T.id,T.uid , S.pname , S.cost , T.total , T.quantity , S.brand , date(T.timestamp) as date FROM transaction T , stocks S WHERE T.pid=S.pid AND T.uid = '$_SESSION[uid]' AND date(timestamp) = '$_POST[date]'";
                } else if (isset($_POST["month"])) {
                    $month = substr($_POST["month"], 5, 6);
                    $year = substr($_POST["month"], 0, 4);
                    $query = "SELECT T.id,T.uid , S.pname , S.cost , T.total , T.quantity , S.brand , date(T.timestamp) as date FROM transaction T , stocks S WHERE T.pid=S.pid AND T.uid = '$_SESSION[uid]' AND month(timestamp) = '$month' AND year(timestamp) = '$year'";
                } else if (isset($_POST["year"])) {
                    $query = "SELECT T.id,T.uid , S.pname , S.cost , T.total , T.quantity , S.brand , date(T.timestamp) as date FROM transaction T , stocks S WHERE T.pid=S.pid AND T.uid = '$_SESSION[uid]' AND year(timestamp) = '$_POST[year]'";
                } else {
                    $query = "SELECT T.id,T.uid , S.pname , S.cost , T.total , T.quantity , S.brand , date(T.timestamp) as date FROM transaction T , stocks S WHERE T.pid=S.pid AND T.uid = '$_SESSION[uid]'";
                }
