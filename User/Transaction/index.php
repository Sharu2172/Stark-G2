    ...
    <table class="table table-responsive text-center table-hover w-100">
        ...
        <tbody>
            <form action="../bill/index.php" method="POST">
                ...
                if (isset($_POST["date"])) {
                    $query = "SELECT * FROM transaction T WHERE T.uid = '$_SESSION[uid]' AND date(timestamp) = '$_POST[date]'";
                    unset($_POST['date']);
                } else if (isset($_POST["month"])) {
                    $month = substr($_POST["month"], 5, 6);
                    $year = substr($_POST["month"], 0, 4);
                    $query = "SELECT * FROM transaction T WHERE T.uid = '$_SESSION[uid]' AND month(timestamp) = '$month' AND year(timestamp) = '$year'";
                    unset($_POST['month']);
                } else if (isset($_POST["year"])) {
                    $query = "SELECT * FROM transaction T WHERE T.uid = '$_SESSION[uid]' AND year(timestamp) = '$_POST[year]'";
                    unset($_POST['year']);
                } else {
                    $query = "SELECT * FROM transaction T WHERE T.uid = '$_SESSION[uid]'";
                }
