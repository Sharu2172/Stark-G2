...
<div class="row g-3 p-2">
    ...
    <div class="col-md-3" id="data">
        <form method="POST" action="" id="dmyform">
            ...
            <input type="text" value="<?php echo $_POST['uid']; ?>" name="uid" hidden>
        </form>
    </div>
    <div class="col-md-3">
...
<div class="float-right row g-3">
    <table class="table table-responsive text-center table-hover w-100">
        ...
        <tbody>
            ...
            if (isset($_POST["date"])) {
                $query = "SELECT *,date(timestamp) as date FROM transaction T WHERE T.uid = '$_POST[uid]' AND date(timestamp) = '$_POST[date]'";
            } else if (isset($_POST["month"])) {
                $month = substr($_POST["month"], 5, 6);
                $year = substr($_POST["month"], 0, 4);
                $query = "SELECT *,date(timestamp) as date FROM transaction T WHERE T.uid = '$_POST[uid]' AND month(timestamp) = '$month' AND year(timestamp) = '$year'";
            } else if (isset($_POST["year"])) {
                $query = "SELECT *,date(timestamp) as date FROM transaction T WHERE T.uid = '$_POST[uid]' AND year(timestamp) = '$_POST[year]'";
            } else {
                $query = "SELECT *,date(timestamp) as date FROM transaction T WHERE T.uid = '$_POST[uid]'";
            }
